...
export default function AdminDashboardPage() {
  ...
  function resetForm(clearStatus = true) {
    ...
    setForm({});
    if (clearStatus) setStatus({ type: '', message: '' });
  }
  ...
  async function handleSubmit(event) {
    ...
    const successMessage = editingRecord
      ? `${activeConfig.title} updated successfully.`
      : `${activeConfig.title} item added successfully.`;
  ...
  async function handleApplicationStatus(row, nextStatus) {
    const payload = buildPayload(resourceConfigs.applications.fields, { ...row, status: nextStatus }, true);

    try {
      setActionBusy(true);
      setStatus({ type: '', message: '' });
      await updateResource('/applications', row.app_id, payload);
      await loadDashboard(false);
      setStatus({
        type: 'success',
        message: nextStatus === 'Approved'
          ? `Application #${row.app_id} approved. Pet #${row.pet_id} is now marked Adopted.`
          : `Application #${row.app_id} marked ${nextStatus}.`,
      });
    } catch (err) {
      const errors = err.response?.data?.errors;
      const firstError = errors ? Object.values(errors).flat()[0] : null;
      setStatus({ type: 'error', message: firstError || err.response?.data?.message || 'Application status update failed.' });
    } finally {
      setActionBusy(false);
    }
  }
        ...
        <section className="soft-card overflow-hidden">
          <div className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap gap-3">
              <button type="button" className="btn-secondary" onClick={() => loadDashboard(true)} disabled={actionBusy}>
                Refresh
              </button>
          ...
          {(Object.keys(form).length > 0 || editingRecord) ? (
            <form onSubmit={handleSubmit} className="mb-6 rounded-[28px] bg-rose-50/70 p-5">
              ...
              <button type="submit" className="btn-primary mt-5" disabled={actionBusy}>
                {actionBusy ? 'Saving...' : editingRecord ? 'Save Changes' : 'Create Record'}
              </button>
            </form>
          ) : null}
                    ...
                    <tr key={row[activeConfig.idKey]}>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex gap-2">
                          {activeSection === 'applications' && String(row.status).toLowerCase() !== 'approved' ? (
                            <button type="button" className="rounded-full bg-emerald-50 px-3 py-2 font-semibold text-emerald-700" onClick={() => handleApplicationStatus(row, 'Approved')} disabled={actionBusy}>
                              Approve
                            </button>
                          ) : null}
                          {activeSection === 'applications' && String(row.status).toLowerCase() !== 'rejected' ? (
                            <button type="button" className="rounded-full bg-red-50 px-3 py-2 font-semibold text-red-600" onClick={() => handleApplicationStatus(row, 'Rejected')} disabled={actionBusy}>
                              Reject
                            </button>
                          ) : null}
                          <button type="button" className="rounded-full bg-rose-50 px-3 py-2 font-semibold text-brand-pink" onClick={() => openEdit(row)} disabled={actionBusy}>
                            Edit
                          </button>
                          <button type="button" className="rounded-full bg-slate-100 px-3 py-2 font-semibold text-slate-600" onClick={() => handleDelete(row)} disabled={actionBusy}>
                            Delete
                          </button>
                        </div>
                      </td>
...
function buildPayload(fields, form, editing) {
  return fields.reduce((payload, field) => {
    ...
    if (!required && (value === '' || value === undefined || value === null)) return payload;

    payload[field.name] = field.type === 'number' && value !== '' && value !== null ? Number(value) : (value ?? '');
    return payload;
  }, {});
}
